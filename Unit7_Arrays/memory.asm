...
global asm_copy, asm_find_address_of, asm_find_index_of, asm_strlen, asm_strcpy
...
; function asm_find_address_of
; searches memory for a given byte
; void * asm_find_address_of( const void * src, char target, unsigned sz);
; parameters:
...
; function asm_find_index_of
; searches memory for a given byte
; int asm_find_index_of( const void * src, char target, unsigned sz);
; parameters:
;   src    - pointer to buffer to search
;   target - byte value to search for
;   sz     - number of bytes in buffer
; return value:
;   if target is found, index of first occurence of byte in given buffer
;   else
;     -1 is returned
; NOTE: target is a byte value, but is pushed on stack as a dword value.
;       
...
asm_find_index_of:
		...
		; could have also called asm_find_address_of
		mov edi, src
		mov al, target
		mov ecx, sz
